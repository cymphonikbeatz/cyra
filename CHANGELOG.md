...
### Changed
- Complete brand identity transition to Cyra, including application icon redesign and updated menu bar glyphs.
- Intel Mac rename compliance audit across all core services, Settings pages, UI components, and documentation.
- Completed Category C linguistic rebrand across all 12 supported localizations with grammatical harmony fixes (Turkish suffixes, Portuguese/German noun/article agreement).
- Refreshed all README and documentation screenshot assets from the live Cyra build at native 2x Retina resolution.
- Removed deprecated community Discord links in favor of direct GitHub project tracking.
- Added signing environment configuration support (`CODESIGN_IDENTITY`) with graceful ad-hoc fallback for local builds.
